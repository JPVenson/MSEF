...
namespace IEADPC.Shell.Services
{
    public class ServicePool : IServicePool
    {
        ...
        private static ServicePool _instance = CreateServicePool();
        ...
        void InitLoading()
        {
            ...
            var enumerable = serviceInternal.Where(s => s.Metadata.Contract == typeof(IApplicationProvider));
            foreach (var lazy in enumerable)
            {
                var val = lazy.Value;
                val.OnStart(ApplicationContainer);
                ManagedLivetimeServicePool.Instance.AddLivetimeService(new Tuple<IService, IServiceMetadata>(lazy.Value, lazy.Metadata));
            }
        }
        ...
        /// </summary>
        public static IApplicationContext ApplicationContainer { get; private set; }
        ...
        /// <a href="http://msdn.microsoft.com/en-us/library/system.componentmodel.composition.importattribute%28v=vs.100%29.aspx">ImportAttribute </a>
        /// </summary>
        /// <param name="part">your <paramref name="part"/> item</param>
        /// <returns>True if the Operation was Successful</returns>
        public bool Import([Required()] ComposablePart part)
        {
            try
            {
        ...
        /// </summary>
        /// <param name="composition">Your <see cref="CompositionBatch"/></param>
        /// <returns>True if the Operation was Successful</returns>
        ...
        /// This Code is Internal and should not be used directly from your code
        /// </summary>
        /// <param name="ignoreDefauld">Ignore all services where the <see cref="IServiceMetadata.IsDefauldService"/> property true is</param>
        ...
        /// <exception cref="NotImplementedException">Thrown when There is no Unique <typeparamref name="T"/> and no Incident Fixer for <typeparamref name="T"/></exception>
        public T GetDefaultSingelService<T>() where T : class, IService
        {
            var exports = GetServiceInternal(false);
            var defaultservice = exports
                .Where(m => m.Metadata.IsDefauldService && m.Metadata.Contract == typeof(T));
        ...
        {
            var firstOrDefault = GetServiceInternal().Where(m => m.Metadata.Contract == typeof(T)).Select(m => m.Value as T).FirstOrDefault();
        ...
        /// </summary>
        /// <typeparam name="T">Your Service interface</typeparam>
        /// <param name="output">The found <typeparamref name="T"/> will be Marshaled back to you</param>
        /// <returns>True if the Operation was Successful</returns>
        ...
        {
            return GetServiceInternal().Where(s => s.IsValueCreated && s.Value == service).Select(s => s.Metadata).FirstOrDefault();
        }
