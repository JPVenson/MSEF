...
namespace JPB.Shell.CommonAppliationContainer.Ribbon
{
    public class RibbonEx : Microsoft.Windows.Controls.Ribbon.Ribbon
    {
        public static readonly DependencyProperty MetadatasProperty = DependencyProperty.Register(
            "Metadatas", typeof (ObservableCollection<IRibbonMetadata>), typeof (RibbonEx),
            new PropertyMetadata(default(ObservableCollection<IRibbonMetadata>), MetadatasChanged, MetadataCoreValidata));
        ...
        private static void MetadatasChanged(DependencyObject dependencyObject,
        ...
        public void OnMetadataChanged()
        {
            ...
            {
                foreach (IRibbonTab generateTab in ribbonProvider.GenerateTabs(Metadatas).OrderBy(s => s.TabIndex))
                {
                    var tab = new RibbonTab();
                    tab.Header = generateTab.Header;
                    foreach (IRibbonGroup ribbonGroup in generateTab.Groups.OrderBy(s => s.GroupIndex))
                    {
                        var group = new RibbonGroup();
                        group.Header = ribbonGroup.Header;
                        foreach (RibbonButton button in ribbonGroup.Buttons)
                        {
